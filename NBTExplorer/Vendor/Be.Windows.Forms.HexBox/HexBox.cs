    ...
    public class HexBox : Control
    {
        ...
        private class KeyInterpreter : IKeyInterpreter
        {
            ...
            protected virtual bool PreProcessWmKeyDown_ShiftDown(ref Message m)
            {
                ...
                var max = _hexBox._byteProvider.Length;
            ...
            public virtual bool PreProcessWmChar(ref Message m)
            {
                ...
                var sw = _hexBox._byteProvider.SupportsWriteByte();
                var si = _hexBox._byteProvider.SupportsInsertBytes();
                var sd = _hexBox._byteProvider.SupportsDeleteBytes();
                ...
                if (Uri.IsHexDigit(c))
                {
                    ...
                    var isInsertMode = pos == _hexBox._byteProvider.Length;
                    ...
                    var newcb = byte.Parse(sNewCb, NumberStyles.AllowHexSpecifier, Thread.CurrentThread.CurrentCulture);
            ...
            private Dictionary<Keys, MessageDelegate> MessageHandlers
            {
                get
                {
                    if (_messageHandlers == null)
                    {
                        ...
                        _messageHandlers.Add(Keys.Left | Keys.Shift,
                            PreProcessWmKeyDown_ShiftLeft); // move left with selection
                        _messageHandlers.Add(Keys.Up | Keys.Shift,
                            PreProcessWmKeyDown_ShiftUp); // move up with selection
                        _messageHandlers.Add(Keys.Right | Keys.Shift,
                            PreProcessWmKeyDown_ShiftRight); // move right with selection
                        _messageHandlers.Add(Keys.Down | Keys.Shift,
                            PreProcessWmKeyDown_ShiftDown); // move down with selection
                        ...
                        _messageHandlers.Add(Keys.ShiftKey | Keys.Shift,
                            PreProcessWmKeyDown_ShiftShiftKey); // begin selection process
        ...
        private class StringKeyInterpreter : KeyInterpreter
        {
            ...
            public override PointF GetCaretPointF(long byteIndex)
            {
                ...
                var gp = _hexBox.GetGridBytePoint(byteIndex);
        ...
        private long FromScrollPos(int value)
        {
            ...
            if (_scrollVmax < max)
                return value;
            var valperc = value / (double)max * 100;
        ...
        private void PaintLineInfo(Graphics g, long startByte, long endByte)
        {
            ...
            var lineInfoColor = LineInfoForeColor != Color.Empty ? LineInfoForeColor : ForeColor;
            Brush brush = new SolidBrush(lineInfoColor);

            var maxLine = GetGridBytePoint(endByte - startByte).Y + 1;

            for (var i = 0; i < maxLine; i++)
            {
                var firstLineByte = startByte + _iHexMaxHBytes * i + _lineInfoOffset;

                var bytePointF = GetBytePointF(new Point(0, 0 + i));
                var info = firstLineByte.ToString(_hexStringFormat, Thread.CurrentThread.CurrentCulture);
        ...
        private void PaintHex(Graphics g, long startByte, long endByte)
        {
            ...
            var counter = -1;
            var intern_endByte = Math.Min(_byteProvider.Length - 1, endByte + _iHexMaxHBytes);
        ...
        private void PaintHexStringSelected(Graphics g, byte b, Brush brush, Brush brushBack, Point gridPoint)
        {
            ...
            var isLastLineChar = gridPoint.X + 1 == _iHexMaxHBytes;
        ...
        private void PaintHexAndStringView(Graphics g, long startByte, long endByte)
        {
            ...
            var isKeyInterpreterActive = _keyInterpreter == null || _keyInterpreter.GetType() == typeof(KeyInterpreter);
            var isStringKeyInterpreterActive =
        ...
        private void PaintCurrentBytesSign(Graphics g)
        {
            if (_keyInterpreter != null && Focused && _bytePos != -1 && Enabled)
                if (_keyInterpreter.GetType() == typeof(KeyInterpreter))
                {
                    if (_selectionLength == 0)
                    {
                        var gp = GetGridBytePoint(_bytePos - _startByte);
                    ...
                    else
                    {
                        ...
                        var startSelGridPoint = GetGridBytePoint(_bytePos - _startByte);
                        var startSelPointF = GetByteStringPointF(startSelGridPoint);

                        var endSelGridPoint = GetGridBytePoint(_bytePos - _startByte + _selectionLength - 1);
        ...
        {
            var gp = GetGridBytePoint(byteIndex);
        ...
        {
            var x = 3 * _charSize.Width * gp.X + _recHex.X;
        ...
        /// <summary>
        ///     Converts the hex string to an byte array. The hex string must be separated by a space char ' '. If there is any
        ///     invalid hex information in the string the result will be null.
        /// </summary>
        ...
        {
            var isByte = byte.TryParse(hex, NumberStyles.HexNumber, Thread.CurrentThread.CurrentCulture, out b);
            return isByte;
        }
        ...
        {
            var currentLine = (long)Math.Floor(_bytePos / (double)_iHexMaxHBytes) + 1;
        ...
        {
            var gb = GetGridBytePoint(_bytePos);
