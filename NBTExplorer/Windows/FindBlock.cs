...
namespace NBTExplorer.Windows
{
    ...
    {
        private readonly CoordinateGroup _groupX;
        private readonly CoordinateGroup _groupZ;

        private readonly DataNode _searchRoot;
        ...
        private DataNode Search(DataNode node)
        {
            ...
            {
                var dirNode = node as DirectoryDataNode;
                if (!dirNode.IsExpanded)
                    dirNode.Expand();

                foreach (var subNode in dirNode.Nodes)
                {
                    var resultNode = Search(subNode);
                    if (resultNode != null)
                        return resultNode;
                }
            ...
            {
                var regionNode = node as RegionFileDataNode;
            ...
            {
                var chunkNode = node as RegionChunkDataNode;
        ...
        {
            var start = region * 32 * 16;
            var end = (region + 1) * 32 * 16 - 1;
        ...
        private void ApplyLocalBlock(CoordinateGroup group, string value, bool primary)
        {
            ...
            if (primary && group.LocalBlock.HasValue)
                ...
                {
                    ApplyChunk(group,
                        ChunkFromLocalBlock(group.Region.Value, group.LocalChunk.Value, group.LocalBlock.Value), false);
                    ApplyBlock(group,
                        BlockFromLocalBlock(group.Region.Value, group.LocalChunk.Value, group.LocalBlock.Value), false);
                }
                else
                {
