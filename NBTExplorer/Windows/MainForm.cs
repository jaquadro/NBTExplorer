...
namespace NBTExplorer.Windows
{
    ...
    {
        private static readonly Dictionary<TagType, int> _tagIconIndex;

        private readonly NodeTreeController _controller;
        ...
        private void _menuItemOpenInExplorer_Click(object sender, EventArgs e)
        {
            ...
            {
                var ddNode = _nodeTree.SelectedNode.Tag as DirectoryDataNode;
                try
                {
            ...
            if ((ModifierKeys & Keys.Control) > 0 && (ModifierKeys & Keys.Shift) == 0)
                ...
                using (var ofd = new FolderBrowserDialog())
                {
                    ...
                    if (ofd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        OpenPaths(new[] { ofd.SelectedPath });
                    }
                }
        ...
        {
            var failCount = _controller.OpenPaths(paths);
            ...
            if (failCount > 0) MessageBox.Show("One or more selected files failed to open.");
        }
        ...
        private void OpenMinecraftDirectory()
        {
            ...
            try
            {
                ...
                if (!Directory.Exists(path)) path = Environment.GetFolderPath(Environment.SpecialFolder.MyComputer);
            ...
            catch (Exception e)
            {
                ...
                try
                {
                    OpenPaths(new[] { Directory.GetCurrentDirectory() });
                }
                catch (Exception)
                {
                    MessageBox.Show(
                        "Could not open current directory, this tool is probably not compatible with your platform.");
                    Console.WriteLine(e.Message);
                    Application.Exit();
                }
            }
        ...
        private bool ConfirmExit()
        {
            if (_controller.CheckModifications())
                if (MessageBox.Show("You currently have unsaved changes.  Close anyway?", "Unsaved Changes",
        ...
        {
            if (_controller.CheckModifications())
                if (MessageBox.Show("You currently have unsaved changes.  " + actionMessage, "Unsaved Changes",
                        MessageBoxButtons.OKCancel) != DialogResult.OK)
                    return false;
        ...
        {
            if (_controller.CheckModifications())
                if (MessageBox.Show("You currently have unsaved changes.  " + e.Message, "Unsaved Changes",
                        MessageBoxButtons.OKCancel) != DialogResult.OK)
                    e.Cancel = true;
        }
        ...
        {
            var selected = _nodeTree.SelectedNode;
            ...
            {
                DisableButtons(_buttonAddTagByte, _buttonAddTagByteArray, _buttonAddTagCompound, _buttonAddTagDouble,
                    _buttonAddTagFloat,
                    _buttonAddTagInt, _buttonAddTagIntArray, _buttonAddTagList, _buttonAddTagLong,
                    _buttonAddTagLongArray, _buttonAddTagShort,
                    _buttonAddTagString, _buttonCopy, _buttonCut, _buttonDelete, _buttonEdit, _buttonPaste,
                    _buttonRefresh,
                    _buttonRename);
                ...
                DisableMenuItems(_menuItemCopy, _menuItemCut, _menuItemDelete, _menuItemEditValue, _menuItemPaste,
                    _menuItemRefresh,
        ...
        private void UpdateUI(IList<TreeNode> nodes)
        {
            ...
            _buttonAddTagByte.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateByteNodePred);
            _buttonAddTagShort.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateShortNodePred);
            _buttonAddTagInt.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateIntNodePred);
            _buttonAddTagLong.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateLongNodePred);
            _buttonAddTagFloat.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateFloatNodePred);
            _buttonAddTagDouble.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateDoubleNodePred);
            _buttonAddTagByteArray.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateByteArrayNodePred);
            _buttonAddTagIntArray.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateIntArrayNodePred);
            _buttonAddTagLongArray.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateLongArrayNodePred);
            _buttonAddTagString.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateStringNodePred);
            _buttonAddTagList.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateListNodePred);
            _buttonAddTagCompound.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.CreateCompoundNodePred);
            ...
            _buttonCut.Enabled = _controller.CanOperateOnSelection(NodeTreeController.Predicates.CutNodePred) &&
                                 NbtClipboardController.IsInitialized;
            ;
            _buttonCopy.Enabled = _controller.CanOperateOnSelection(NodeTreeController.Predicates.CopyNodePred) &&
                                  NbtClipboardController.IsInitialized;
            ;
            _buttonPaste.Enabled = _controller.CanOperateOnSelection(NodeTreeController.Predicates.PasteIntoNodePred) &&
                                   NbtClipboardController.IsInitialized;
            ;
            _buttonFindNext.Enabled = _controller.CanOperateOnSelection(NodeTreeController.Predicates.SearchNodePred) ||
                                      _searchState != null;
            ...
            _menuItemMoveDown.Enabled =
                _controller.CanOperateOnSelection(NodeTreeController.Predicates.MoveNodeDownPred);
        ...
        private ToolStripDropDown BuildRecentEntriesDropDown(StringCollection list)
        {
            ...
            {
                var item = new ToolStripMenuItem("&" + (menu.Items.Count + 1) + " " + entry);
                item.Tag = entry;
        ...
        private void _menuItemRecentPaths_Click(object sender, EventArgs e)
        {
            ...
            OpenPaths(new[] { item.Tag as string });
        }
        ...
        {
            var form = new FindBlock(_nodeTree.SelectedNode.Tag as DataNode);
            if (form.ShowDialog() == DialogResult.OK)
                if (form.Result != null)
                {
